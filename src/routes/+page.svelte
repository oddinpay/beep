<script lang="ts">
  ...
  const description = "Real-time and historical data on Beep system performance.";
  ...
  json.subscribe((msg: any) => {
    ...
    const existing = probeMap[probe.id];
    const order = Number.isFinite(index)
      ? index
      : (existing as any)?.__order ?? Number.POSITIVE_INFINITY;

   const nextProbe = { ...(existing ?? {}), ...probe, uptime90: sla?.uptime90, __order: order };

    probeMap[probe.id] = nextProbe;
  ...
  let mockData = $derived.by(() => {
    ...
    return limitedProbes.map((probe) => {
      ...
      const statesList = Array.isArray(probe?.state) ? probe.state : [probe?.state];
  ...
  let monitors = $derived.by(() => {
    ...
    const merged = new Map<string, { title: string; description: string; status: string }>();
    ...
    },
    {
      ...
      entries: [
...

        {
          time: "Sep 22, 2025 13:05 UTC",
          status: Indicators.Identified,
          description: "We are identifying reports of increased errors on iDeal payments.",
        },
        ...
        <article id="content" class="markdown-body p-5 max-w-5xl">
          <div class="flex flex-col justify-center">
            <div class="py-25">
              <div class="wrapper-ui">
                <div class="child-wrapper-ui">
                  <div class="headline-container">
                        ...
                        {:else}
                          <div role="status" class="profile shimmer w-20 h-20 bg-gray-300"></div>
                        {/if}
                    ...
                    >
                      {overallStatus === 'up' ? 'All Systems Operational' : overallStatus === 'warn' ? 'System Outage Detected' : overallStatus === 'down' ? 'Critical Issues Detected' : 'No Status Available'}
                    </h1>
              ...
              <Tabs value={activeTab} class="items-left p-5" onValueChange={handleChange}>
                <TabsList
                ...
                >
                  ...
                  <TabsContent value={t}>
                    {#if i === 0}
                          {#if submitMonitors  && monitors.length === 0 }
                              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24"><circle cx="18" cy="12" r="0" fill="currentColor"><animate attributeName="r" begin=".67" calcMode="spline" dur="1.5s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="12" cy="12" r="0" fill="currentColor"><animate attributeName="r" begin=".33" calcMode="spline" dur="1.5s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle><circle cx="6" cy="12" r="0" fill="currentColor"><animate attributeName="r" begin="0" calcMode="spline" dur="1.5s" keySplines="0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8;0.2 0.2 0.4 0.8" repeatCount="indefinite" values="0;2;0;0"/></circle></svg>   
                          {:else}
                          ...
                          <div class="status-page">
                            <div class="left">
                              <h3>System status</h3>
                              {#if submitMonitors && showSpinner && monitors.length === 0}
                    ...
                    {:else if i === 1}
                        {#if incidents.length === 0 && maintenances.length === 0}
                            <p class="p-10 text-gray-600 text-center" style="font-size: 16px">
