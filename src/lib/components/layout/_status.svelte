<script lang="ts">
  ...
  const json = source("/sse").select("message").json<any>();

  json.subscribe((msg: any) => {
    const probe = msg?.payload?.probe;
    if (!probe?.id) return;

    const existingProbe = $probeMap[probe.id];
    if (existingProbe) {
      Object.assign(existingProbe, probe);
    } else {
      $probeMap[probe.id] = probe;
    }
  });
  ...
  let monitors = $derived.by(() => {
    ...
    for (const api of dataToShow) {
      ...
      let status = (api as any).state ?? 'warn';


      console.log("Assigning status for", api?.name, ":", status);
